(() => {
    ...
    async function updateQuantity(
        ...
        if (!Number.isInteger(variantId) || !input) {
            return;
        }
        ...
        const maximum =
            Number.parseInt(input.max, 10) || 1;
    ...
    page.addEventListener("click", async event => {
        ...
        checkout.textContent =
            "Đang kiểm tra giá và tồn kho…";

        try {
            const result = await api.requestJson(
                "/cart/prepare-checkout",
                {
                    method: "POST"
                });
